    ...
    public partial class Form_Student : Form
    {
        ...
        private void btn_hk_Click(object sender, EventArgs e)
        {
            ...
            if (hk == hk1)
            {
                ...
                string sql = "SELECT ROUND(AVG(DiemTrungBinh), 2) AS DiemTrungBinhTong,    CASE       \r\nWHEN ROUND(AVG(DiemTrungBinh), 2) >= 9 THEN N'Xuất sắc'      \r\nWHEN ROUND(AVG(DiemTrungBinh), 2) >= 8 AND ROUND(AVG(DiemTrungBinh), 2) < 9 THEN N'Giỏi'       \r\nWHEN ROUND(AVG(DiemTrungBinh), 2) >= 7 AND ROUND(AVG(DiemTrungBinh), 2) < 8 THEN N'Khá'      \r\nWHEN ROUND(AVG(DiemTrungBinh), 2) >= 5 AND ROUND(AVG(DiemTrungBinh), 2) < 7 THEN N'Trung bình'       ELSE N'Yếu'   END AS HocLuc FROM (    SELECT (DiemMieng + Diem15Phut + Diem1Tiet * 2 + DiemThi * 3) / 7 AS DiemTrungBinh   FROM Diem   WHERE Diem.MaHocKy = 'HK001') AS DiemTrungBinhCacMon ,Diem Where MaHocSinh = ' "+ username + "'";
